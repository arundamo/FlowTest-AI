        ...
        <div className="flex items-center gap-2">
          {isEditing ? (
            ...
            <>
              ...
              <div className="flex items-center bg-slate-900 border border-slate-700/80 rounded-lg p-0.5 text-xs font-mono">
                <button
                  type="button"
                  onClick={() => onToggleBrowserMode?.('headless')}
                  ...
                  title="Headless Mode: background execution"
                >
                ...
                <button
                  type="button"
                  onClick={() => onToggleBrowserMode?.('headed')}
                  className={`px-2 py-1 rounded text-[11px] font-medium flex items-center gap-1 transition-all ${
                    browserMode === 'headed'
                      ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 font-semibold shadow-sm'
                      : 'text-slate-400 hover:text-slate-200'
                  }`}
                  title="Headed Mode: view live browser window during execution"
                >
              ...
              {targetUrl && (
                <button
                  ...
                  className="p-1.5 rounded-lg bg-slate-800/80 hover:bg-slate-800 text-slate-300 text-xs transition-colors border border-slate-700/60"
                  title="Open test target in standalone 1280x720 window"
                >
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-lg disabled:opacity-50 text-white text-xs font-semibold flex items-center gap-1.5 transition-all shadow-md ${
                  browserMode === 'headed'
                ...
                title={
                  browserMode === 'headed'
                    ? 'Trigger Phase 3: Run headed Chromium test with live interactive viewport inspection'
                    : 'Trigger Phase 3: Run headless Chromium test execution'
                }
              >
                {isRunningTest ? (
                  <>
                    <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                    <span>Executing ({browserMode === 'headed' ? 'Headed' : 'Headless'})...</span>
                  </>
                ) : browserMode === 'headed' ? (
                  <>
